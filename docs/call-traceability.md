...
| SO3 | ≥ 1 functioning prototype on ActivityPub or AT Protocol | **Implemented** — federating server, actors, inbox/outbox, WebFinger, HTTP Signatures, NodeInfo, plus circles (private/topic/local) as the room model | `src/federation/`, `src/domain/circles.js`, `tests/federation.test.js`, `tests/circles.test.js` |
...
| Look and feel appealing to young audiences | The lamb design system, server-rendered, with light/dark, low-stimulus and plain-language modes | `src/web/style.js`, `design/lamb.html` |
| Rooms with a known audience | Circles: private / topic / local, membership-gated, kind determines visibility | `circles.test.js` |
...
1. **Youth Panels** — the `panel` circle kind exists in the data model, but the
   procedure (five phases, timing, paired viewpoints, published protocol) is not
   built.
2. **Onboarding** — the welcome-circle flow with justified circle suggestions.
3. **Media uploads** — the data model and alt-text rule are in place, but there
   is no upload pipeline or storage backend.
4. **Direct messages** — `dmFrom` is enforced as a preference but no DM feature
   exists yet.
5. **Delivery queue robustness** — retries are linear and in-process; a real
   deployment needs backoff, a dead-letter path and a separate worker.
6. **Instance-level moderation** — per-instance blocks, allowlist federation and
...
7. **Mention parsing** — `mentioned` policies match on the handle in the body;
   a proper mention table is needed for reliability.
8. **AT Protocol bridge** — the feed interface is shaped for it; the bridge
   itself is not written.
9. **Rate limiting and abuse defences at the HTTP layer.**
10. **i18n** — the interface is German-only; the co-creation process runs in eight
   languages, so a string catalogue is a prerequisite for wave 3.
